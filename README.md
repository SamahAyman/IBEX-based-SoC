...
As detailed in [openLANE](https://github.com/efabless/openlane)'s documentations, each design needs a config file. So, we wrote our config file to make our hardene MACRO compatible with Caravel, and to reach the optmal hardening results. 

the config file is included in **** directory

we did much experimentation to find out the optimum config settings ... the details of this is detailed in MS2 Report.pdf where we have a table with different options and the corresponding results. 
...
SoCGen included a number of systems. Out of them, *N5-based-SoC* was the closet to our *IBEX-based-SoC*. So, we used it as a golden model to check the correction of our model.
